---
title: 'Visualizing Blood Bowl Road to Malta Tournament rosters'
...
Bad roster choices will put you at a disadvantage even before the actual game has started. 
So let's check out what the pro's are using!

# Online Blood Bowl Tournaments on FUMBBL

Blood Bowl can also be played online. Here we focus on [FUMBBL.com](https://fumbbl.com/), consisting of a Java app that contains the actual Blood Bowl game client, linked to a website that performs match making, calculates rankings, provides a forum for discussions etc. 
...
Tthis blog post main goal is to share the analysis results. [Check my Github](https://github.com/gsverhoeven/hugo_source/tree/master/content/post/fumbbl_rosters_post) for the technical stuff (R/Python web scraping and visualization of FUMBBL data). 

I'll discuss the most popular races, and leave the rest for the interested reader. I created [a PDF rosterbook](link/to/pdf) containing all 29 different races that participated in the tournament. For each race I made a plot (a clustered heatmap) displaying the roster as a colorcoded matrix, where teams that are similar (based on a cost weighted similarity score) are put side by side. To give some information about the performance of the various builds, I added the tournament points scored behind the coach name. Of course, roster build is only one factor influencing tournament results, and by far not the most important, so we have to keep that in mind.

# Top 10 races in Road To Malta

The Road to Malta tournament feautured 46 teams of 4 coaches each, supplying a total of 184 rosters for our analysis.
...
## Underworld Denizens: star players galore!
...
![](fumbbl_rosters_post/uw_roster_malta.png){width=200%}
The roster with the best performance (by Sandune) was the Varag plus Troll build.

## High elf: a shitload of skills

UW shared the top spot for most popular team with High Elves. High Elf's popularity is often attributed to their low tier placement, allowing for 8 skills, on a 12 player line up.
...
![](fumbbl_rosters_post/he_roster.png){width=200%}

## Necromantic Horror

Necro was in tier 2, allowing for seven skills.
Skill wise, well, a lot of guard and block!
For Necro, there where three variants popular:

* Single werewolf, 6 zombies, 3 rerolls
* Two werewolves, 4 zombies, 2 rerolls
* Two werewolves, 3 zombies, 3 rerolls

Performance-wise, there is no clear winner here. 

![](fumbbl_rosters_post/nh_roster_malta.png){width=200%}

## Shambling Undead

The Shambling Undead rosters are all surprisingly similar: 2 mummies with guard, 2 wights (one guard, one tackle or mighty blow), 4 ghoul runners (two with block, 1 wrestle), and three rerolls. The only noticable variation is whether to complement that lineup with 5 zombies, 4 zombies plus 1 skeleton, or 3 zombies plus 2 skeletons. Performance wise there is little to guide that last judgement, as all three variants performed similar.

![](fumbbl_rosters_post/su_roster_malta.png){width=200%}

## Dark Elf: to assassinate or not
...
![](fumbbl_rosters_post/de_roster_malta.png){width=200%}

## Chaos Dwarf: with or without minotaur
...
The highest scoring build by Liam has an uncommon skill choice, with guard on the bull centaurs and two blockers with Mighty Blow.

![](fumbbl_rosters_post/cd_roster_malta.png){width=200%}
...
![](fumbbl_rosters_post/sk_roster_malta.png){width=200%}

## Lizardmen

For lizardmen coaches life is simple. 6 saurusses, 5 skinks, krox and an apo. Put block on all saurusses, done.
That leaves time to bicker on about bigger questions, such a whether to take a single **Chameleon skink** :)
...
![](fumbbl_rosters_post/lz_roster_malta.png){width=200%}
...
![](fumbbl_rosters_post/orc_roster_malta.png){width=200%}

## Wood Elf

Wood elves always take the treeman, two wardancers, and at least two catchers.
...
![](fumbbl_rosters_post/we_roster_malta.png){width=200%}
