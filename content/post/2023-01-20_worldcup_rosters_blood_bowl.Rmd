...
res <- df_rosters %>%
  ...
  group_by(player_id, team_id, coach_name, roster.name, tot_cost) %>% # remove two rows per player (skill stacking)
  summarise() %>%
...
gp <- ggplot(df, aes(x = factor(coach_name), y = reorder(paste(position, nr), -sort_order))) +
  ...
  gp
```
PM create roster structure with sort order, and player cost. Daar kunnen we clustering op doen.
En lege posities mee creeren, en positions kleuren cf skill bands.
