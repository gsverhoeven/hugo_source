...
Although there are Azure-cli commands to stop, start and dellocate the VM, I typically use a shell script `deploy_vm_ubuntu.sh` to bring it up, leave it running while I need it, and then tear it down using another script `delete_vm_ubuntu.sh`. This script simply removes the Resource group including everything that is contained within:
...
## TODO Generate password interactively
...
As I am writing this blog, the VM is up and running, and every few hours an IP address get banned.
