...
Because the default address is **dynamic**, create a named DNS entry with the `--domain-name-label` parameter. The following example creates a public IP named `myPublicIP` with the DNS name of `mypublicdns`. Because the DNS name must be unique, provide your own unique DNS name:

```
az network public-ip update -n StanDEV-ip --dns-name standev -g myRG
```

This results in a `fqdn` of `standev.northeurope.cloudapp.azure.com`

```
nslookup  standev.northeurope.cloudapp.azure.com
Server:		127.0.0.53
Address:	127.0.0.53#53

Non-authoritative answer:
Name:	standev.northeurope.cloudapp.azure.com
Address: 68.219.248.112
```
We have a static IP, is more expensive. But more secure.
SKU is standard. Basic Dynamic is cheapest.
```
az network public-ip update -g myRG -n StanDEV-ip --dns-name standev --allocation-method Dynamic
```
...
We'll detach the public IP address from the NIC.
```
resourceGroupName=myRG
nicName=standev363_z1
publicIp=StanDEV-ip
...
----------  ---------------  -----------  -------  --------------  ----------------------  -------------------
StanDEV-ip  myRG             northeurope  1        68.219.248.112  4                       Succeeded
```
...
Most tutorials skip a few components (mostly the networking part), as these are automatically created when creating a VM.
Since we want to know all the resources involved, we'll create them manually here.
...
PM Create an association between the network security group and the network interface
...
The VM needs a public IP address, so we can access it through the internet.
Azure assigns one automatically and returns it as JSON. However, this is a dynamic IP address?

alias / sku ius "20_04-lts"
...
We can also use `run-command` to install software.
...
ERROR: Virtual machine extension 'Microsoft:2EGuestConfiguration-ConfigurationforLinux' is not supported for template export.
ERROR: Virtual machine extension 'Microsoft:2EEnterpriseCloud:2EMonitoring-OmsAgentForLinux' is not supported for template export.


# Cloudinit Installing software on the VM

We can install software using cloud init or just use run-command.
