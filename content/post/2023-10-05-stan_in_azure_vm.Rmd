...
## Using Azure CLI to interact with our Azure account
...
To have a reasonable fast software install I found we need at least 8GB of mem and 2 vCPUs.
...
The SKU of the storage account with which to persist the VM.

Use a singular sku that would be applied across all disks, or specify individual disks. 

```
Usage: [--storage-sku SKU | --storage-sku ID=SKU ID=SKU ID=SKU...], where each ID is "os" or a 0-indexed lun. 
```
...
For the 2cpu 8GB we get 30GB OS disk and 16GB tmp disk. 

I did not go so far as to attach a data disk. So we need to make sure that work we do on it is pushed to e.g. Github, because if we tear the VM down, the user data is lost as well.
...
Hackers' will be using a list of azure IPs and will attempt brute force SSH to gain access to your server.

We take the following steps to secure our VM from outside attacks:

* All ports are closed except SSH and RDP
...
fail2ban will dynamically block brute force attacks on the remote system. It is pre-configured for the default ssh port 22/tcp. 
```
