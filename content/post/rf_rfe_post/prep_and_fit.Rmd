...
Note: this R-squared uses the correlation based definition.
...
Lets look at the variability of R2 when using 100 trees for each forest.
Repeat 30 times.
Since the R squared is based on OOB predictions, we mix 2 things here: the model quality is less with less trees, AND the sample on which we calculate R2 is lower with less trees.
...
Clearly the variability in R2 is much reduced with 1000 trees.
It is reasonable to compare the bootstrapped R2 with repeated runs, since the variation in OOB samples will be similar to the bootstrapping sampling variation.

```{r}
summary(bootstrap_r_squared(df_friedman1_N500$y , rf.fit$predictions))

summary(bootstrap_r_squared_alt(df_friedman1_N500$y , rf.fit$predictions))
...
abline(a = 0, b = 1)
```
So why is the R2 of ranger so low here? It seems to perform pretty well judged by the correlation.
