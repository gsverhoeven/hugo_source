...
`Puma32H` is noteworthy in that it is a classic ML dataset for a simulated PUMA 560 robotarm, that contains mostly irrelevant features (30 out of 32) [@geurts_etal06]. For details on the other two datasets see the `prep_and_fit.Rmd` notebook on my Github.
...
With so many irrelevant variables, we might need to average over more than the default `num.trees` splits 
