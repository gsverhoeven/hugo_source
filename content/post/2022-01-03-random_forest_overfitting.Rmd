...
In a recent (2019) talk on Random Forests (a tree-based model that performs well in many settings), Prof Marvin Wright discusses the common claim is that "Random Forest works well on high-dimensional data". High-dimensional data is related to the ratio between number of observations and number of variables. Data is called high-dimensional if there are many variables relative to the number of observations. This is common in genetics, when we have say complete genomes for only a handful of subjects. The suggestion is that RF can be used on small datasets with lots of (irrelevant / noise) features without having to do variable selection first. 

To check this claim, Wright shows that RF performance is unaffected by adding 100 noise variables to the `iris` dataset, a famous example classification problem with three different species. Because RF uses decision trees, it performs "automatic" feature (predictor / variable) selection, as it is part of the model building process. This property of the algorithm is used to explain this result. A tree model will simply ignore the noise predictors and choose the relevant predictors instead. 
...
# What to do if there are many noise variables and only a few are relevant?
...
The classic book *Applied Predictive Modeling* by Kuhn and Johnson (2013) contains a similar experiment showing the negative effects of including many irrelevant features in a Random Forest model (chapter 18). However, instead of tuning RF, they suggest **removing the irrelevant features altogether**. 

In this blog post I show that if we gradually increase the amount of **noise** (irrelevant features) relative to the **signal** (relevant features), at some point the RF tuning approach no longer is able to achieve optimal performance. Under such (possibly extreme) circumstances, RF feature selection keeps performing well, filtering out the signal variables from the noise variables. 
...
Finally, we echo others (Kuhn, Ambroise) in stressing the importance of using a special nested cross-validation loop to perform the feature selection and performance assessment, especially when a test set is not available. To drive the point home, I have scrambled the $y$ variable we wish to predict, and show that if we retrain the model after feature selection, we get cross-validated R-squares of 5-10%, purely based on noise variables.
...
In this blog post, we use `mtry` as the only tuning parameter of Random Forest. In the three regression examples of Probst et al. where tuning showed a large difference (see figure above), I have found that tuning only `mtry` was sufficient to get close to optimal performance.
...
I played around a bit with the `min.node.size` parameter, for which often the sequence 5,10,20 is mentioned to vary over. Setting this larger should reduce computation, since it leads to shorter trees, but for the datasets here, the effect is on the order of say 10% reduction, which does not warrant tuning it. I left this at its default of 5 for regression and x for classification.

Marvin Wright points to results from Probst 2019 that show `sample.fraction` to be an important parameter as well. This determines the number of samples from the dataset to draw for each tree. I use the default setting from `ranger()` which is to sample with replacement, and to use all samples for each tree, i.e `sample.fraction = 1`.  
...
The procedure works as follows (see for detailed documentation and examples [here](url/to/caret)):
First we grow a random forest where we track feature importance using the "permutation importance" method. This produces a ranking of all predictors. Subsequently, we remove chunks of predictors, starting with the least important ones. To properly assess out-of-sample performance, this is done 10 times, each time on 9 parts of the data. The tenth part is used to measure the performance on data not used to select the features. For the final model, the number of features to retain is determined by combining the results of the ten separate procedures, and in the end a final model is fitted on all the data, with the subset of features determined again by the performance of each feature in the 10 different procedures.

Typically, as we start removing irrelevant features, performs either stays constant or even increases until we reach the point where features that are predictive of the outcome are getting removed. Note that we do not tune the mtry variable of RF in this procedure. Empirically, it has been observed that this leads to only marginal improvements (Kuhn, feature selection book, Svetnik paper 2004). Conceptually, tuning mtry is a way to reduce the effect of irrelevant features. Since we are applying a procedure to remove the irrelevant features, it makes sense that it has little benefit here.
