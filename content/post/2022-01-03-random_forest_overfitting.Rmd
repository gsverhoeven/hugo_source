...
# OpenML curated datasets

For this blog post we need datasets to demonstrate the behavior of RF and the various packages we use to train and tune the models. In particular, I needed a dataset that is not too big, nor too small, and where Random Forest tuning has a substantial effect. Via the [@probst] paper on RF tuning, I found [OpenML.org](https://new.openml.org/). This is a website dedicated to reproducible, open ML, with a large collection of datasets, focussed on benchmarking and performance comparison. This is a perfect match with our goal here of finding the optimal model using RF. So far, in their publications the focus is on classification datasets, but from the `tuneRanger` [Github repo](https://github.com/PhilippPro/tuneRanger) I found a suite of 29 regression datasets, comparing tuned ranger with default ranger. Indeed, most datasets hardly improve upon tuning, but a few did, and three stood out, with R-squared increases of 6%, 7% and 25% point increase. The 25% point increase was in a synthetic dataset called `puma32H`, going from 67% to 93% after tuning. In all three cases, setting `mtry` to its maximum value gave me close to optimal performance. 
...
# urther details of the Pumadyn data sets can be found in Ghahramani
...
PM deze zijn wel upward biased, moeten op een test set of via RFE kijken, niet via CV.
...
While working on this post, I gradually started to realize that hyperparameter tuning of Random Forest hardly ever results in substantial performance differences, compared with just using the default values [@probst19]. The gains are typically in the order of a few percent improvement in performance metrics, for example increasing the R-squared from 85% to 88%. As with most stuff in ML, it is difficult to claim this in general, 
...
[@vepnik04] We have never yet observed a case where the perfor-
mance actually improves as variables are reduced
