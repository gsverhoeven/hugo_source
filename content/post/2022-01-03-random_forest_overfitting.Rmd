...
75% of the data was used for training the models, and 25% of the data was used for testing the final model performance. In this blog post, we use `mtry` as the only tuning parameter of Random Forest, and choose "variance" as the splitrule (alternative is "extratrees" which is more recent but to my knowledge not necessarily better, update it is better because it has lower computational cost!)
...
As this is procedure that drops predictors that do not correlate with the outcome, we have to be extremely carefull that we end up with something that generalizes to unseen data.
...
## Do the work (PM done on 100 samples)
...
fullrun <- 1
outdir = "rf_rfe_post/"
target <-  paste0(outdir, c("rfe_rf_permute_tmp1234.rds"))
permutate_y <- TRUE
