...
From the article by de Korte et al:

```
Random Forest is to a large extent immune to the presence of irrelevant variables in the model. However, if more irrelevant variables are added to a Random Forest model, at some point its performance will be adversely affected. We therefore performed recursive feature elimination (RFE) with Random Forest for all models in order to obtain models with optimal predictive performance. The rfe() function of the caret package in R was used, with the rangerFuncs set of helper functions (Kuhn book)3. Permutation importance was used as the variable importance mode. The Root Mean Squared Error (RMSE) was the performance metric used to choose the optimal subset size of the variables. Ten different subset sizes were tested by the RFE. These ranged from the first to the tenth decile of the number of variables when the model contained more than 10 variables; for models containing less than or exactly 10 variables, all possible subset sizes were considered.

For the analyses reported in this paper, no variable selection was performed, as random forest is to a large extent immune to the presence of irrelevant variables in the model. However, as more irrelevant variables are added to a random forest model, at some point performance will drop [@hastie_elements_2009; @genuer_variable_2010]. For our main results (the predictive accuracies on the total study sample), we checked the impact of irrelevant variables using recursive feature elimination (RFE) (SDC eTable 2). We found that the impact was small.
```
...
RFEctrl <- rfeControl(functions = rangerFuncs,
                   method = "repeatedcv",
                   rerank = F,
                   returnResamp = "all",
                   number = 10,
                   repeats = 1, # 3x 10 folds?
                   verbose = TRUE,
                   saveDetails = T)

```

# Permutate y settings

We can do a robustness check if the procedure works by permutating the `y` variable and running the same analysis again.
...
if(fullrun){
  ...
  if(permutate_y) solTrainY <- solTrainY[sample(1:length(n_rows), size = n_rows, replace = F)]
  
  for(run_nr in 1:n_perm_runs){
    ss <- sample(1:n_rows, size = n_rows, replace = FALSE)
    
    finalModel <- RunRFEandTuneRF(solTrainXtrans_spur[ss,], solTrainY[ss], ctrl_object = RFEctrl, run_nr = run_nr)
  }
}
# wegschrijven finalModel
```
```{r}
finalModel
```
...
testResults <- rbind(testResults,
                     data.frame(obs = solTestY,
                     RFtune2 =  predict(finalModel, solTestXtrans_spur), n_spurious = -1))
```
...
res
```
So we find that the RFE procedure identified a model with 
