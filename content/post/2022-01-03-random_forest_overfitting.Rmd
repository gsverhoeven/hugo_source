...
ggplot(fit_list %>% filter(ds_group == "df_iris" & algo == "ranger_mtry_default"), 
  ...
  geom_point() +
  geom_label(aes(label = paste0(round(performance*100,0), "%")), nudge_y = -0.02, nudge_x = +0.1) +
  geom_segment(aes(xend = algo, y = 0, yend = performance*100)) +
  coord_flip() +
  facet_wrap(~ ds_name) +
  scale_y_continuous(expand = c(0.2,0))+
  labs(y = "Accuracy (% correct)", x = "", title = "Adding 100 noise variables to the iris dataset")
```
...
ggplot(fit_list %>% filter(ds_group == "df_friedman1" & algo %in% c("ranger_mtry_default")), 
       aes(x = reorder(algo, performance), y = performance*100, col = factor(algo))) +
  geom_point() +
  geom_label(aes(label = round(performance*100,0)), nudge_y = -0.02, nudge_x = +0.1) +
  geom_segment(aes(xend = algo, y = 0, yend = performance*100)) +
  coord_flip() +
  facet_wrap(~ ds_name) +
  scale_y_continuous(expand = c(0.2,0))+
  labs(y = "R-squared (%)", x = "", title = "Adding increasing amount of noise to the Friedman 1 dataset")
```
