...
We use repeated k-fold Cross Validation as resampling method.
...
if(fullrun){
  ...
  for(run_nr in 1:n_runs){
    ss <- sample(1:n_rows, size = 50, replace = FALSE)
    
    finalModel <- RunRFEandTuneRF(solTrainXtrans_spur[ss,], solTrainY[ss], rfe_ctrl = RFE_ctrl, train_ctrl = train_ctrl)
  }
}
