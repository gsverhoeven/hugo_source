...
ggplot(fit_list %>% filter(ds_group == "df_iris" & algo == "ranger_mtry_default"), 
...
*Figure created by Philippe Probst and reproduced from his TuneRanger Github repository*
...
```{r, fig.width= 8, fig.height= 3}
ggplot(fit_list %>% filter(ds_group == "openML" & algo %in% c("ranger_mtry_default", "ranger_mtry_max")), 
       aes(x = reorder(algo, performance), y = performance, col = factor(algo))) +
  geom_point() +
  geom_label(aes(label = round(performance,2)), nudge_y = -0.02, nudge_x = +0.1) +
  geom_segment(aes(xend = algo, y = 0, yend = performance)) +
