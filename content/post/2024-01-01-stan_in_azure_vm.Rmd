---
title: 'Rstudio & Stan on a shoestring in the Azure cloud using Cloud-init'
author: "Gertjan Verhoeven"
date: '2024-01-01'
summary: Automating deployment of a Linux R/Stan development environment in the Azure public cloud.
...
header:
  image: headers/cloud_banner.png
  preview: no
---
...
After weighing my options, I chose to write Linux shell scripts that use the **Azure command-line interface (Azure CLI)** to deploy a Linux VM configured using **Cloud-init**. While doing so I discovered that being able to test changes locally first before deploying to the cloud speeds up development significantly. 
...
* I chose **Cloud-init** as it has emerged as the standard wayu to configure Linux cloud images across all cloud platforms
...
So how much do we end up paying for this low cost cloud computing data analytics VM?
...
If we would bring it up and keep it on for a whole day we pay around 6-7 euro's a day.
If we only use it for 8 hours we would bring it down to 3 euro per day.
...
To control the flow of traffic in and out of your VMs, we use a so-called **network security group**. The following example uses `az network nsg create` to create a network security group named `myNSG`:
...
For this we use **Cloud-init**. It typically runs directly after a cloud instance is created.
It is cross-platform across the public cloud, so in theory we should be able to easily move our instance from one cloud provider to another. Already in the early days of cloud computing (2007), where it was mostly about AWS EC2, people recognized the need to automatically configure AWS instances using user defined scripts. Around that time Cloud-init was born (it was called `ec2-init`  at that time).
...
## Testing cloud-init on the local file system
As we are developing our cloud config script, it nice to be able to test it locally before deploying the whole shabam to the Azure cloud, as this is slow (you don't want to wait 10 minutes to discover you made a typo).
Enter the QEMU virtualizer, that can act as a virtualization host to run the instance locally.
...
These scripts are contained within the `cloud_init_cfg.yaml` file to be written to disk within the instance during init:
...
Also these sites that document "bad IPs" that are known culprits of unauthorized scanning for open ports:

* `stamparm/ipsum` Github repo with bad IPs, that actively scan. 
* `abuseipdb.com`
...
After running the deploy script, we can ssh into the VM and for example verify that cloud-init is still running:
