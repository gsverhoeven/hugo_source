...
summary: To play [Blood Bowl online on FUMBBL.com](https://www.fumbbl.com), a Java
  client is used that works with Java Web Start. On my Ubuntu linux system,
  ...
  after a Ubuntu software update, and might be helpful for others encountering the
  same issues.
...
The "forensics" below were directed to getting Java 8 in the form of openjdk-8 on my Ubuntu systems, and getting the FFB to work on that.
However, in the meantime I also figured out why the FFB client stopped working on Java 11, and how to fix that. 

From the openJDK schedule we can see that both Java 8 and 11 in the form of OpenJDK 8 and 11 are supported until sept 2026, so we have at least four more years of online Blood Bowl :-)

# What is up with GNOME_DESKTOP_SESSION_ID?

SInce playing online Blood Bowl on Ubuntu, I just did `unset -v GNOME_DESKTOP_SESSION_ID` and started playing.
However, since the latest openjdk-11, this stopped working, and I decided to look into it.
...
It turns out that Java tries to detect whether it is on a Gnome / GTK system. If GTK is detected, it changes its "Look and feel" of the FFB client: widgets, creating the white bar below, and uses a different font. 

Previously, by unsetting the environment variable `GNOME_DESKTOP_SESSION_ID`, we could prevent the client from detecting GTK.
...
I then installed the `openjdk-8-jre-headless` package:
...
So if we want `icedtea`, we also get `openjdk-11` as a hard dependency.
...
Indeed, starting the FUMBBL client with `javaws` chooses openjdk 11 automatically.
...
```
This gives us the Blood client with the larger font and white bar covering the lower part of the screen.

At first I played with the `JAVA_HOME` variable, pointing it to `/usr/lib/jvm/java-8-openjdk-amd64`. This does have effect on `javaws`, but it gives an error:
...
```
So no messing around with `JAVA_HOME`.

The trick in the end was to change the symbolic link in `/usr/lib/jvm/default-java` to point at `java-8-openjdk-amd64` instead of pointing at `java-11-openjdk-amd64`.
...
And finally ... we have a working FUMBBL client again, without the white bar! Hurrah!
As long as `openjdk-8-jre` stays available on Ubuntu universe, I think were good.
Judging from [package.ubuntu.com](https://packages.ubuntu.com/search?keywords=openjdk-8-jre), this package is broadly available, up to the latest Ubuntu versions, so fingers crossed.
