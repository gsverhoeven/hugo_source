...
**Update**: Turns out i had some remaining issues with sound. When using the replay functionality, some sounds work, and some sounds don't, and throw a ` could not get audio input stream from input URL Unsupported audio resource!` error. This is using either openJDK 8 or 11. When spectating these sounds do work.  On my Ubuntu 20.04 system, using replay, I get a `TinySound not initialized` error, and sound does not work at all.
