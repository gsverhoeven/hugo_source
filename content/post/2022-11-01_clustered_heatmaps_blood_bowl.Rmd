...
Using data analysis, we can compare teams on match performance statistics and see which teams cluster together. From this clustering we might learn something, for example that teams with similar stats might also have a similar play style. Teams with similar stats can still be different on some aspect not captured in the data, but let's just have a look at what we get! 
...
The common way to group Blood Bowl teams is to look at the balance between primary skill access to **Agility** (Ball handling) and **Strength** ("bashing" of the opponent). 
...
ggplot(df_agg, aes(x = Blocks, y = Completions, size = size, col = race_type)) +
         geom_point() +
  scale_size_area() +
  expand_limits(y=-0.5) +
...
The passing (distance) statistic is surprisingly correlated with the number of successful passes (Completions).
