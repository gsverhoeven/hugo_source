...
summary: If you graph the numbers of any natural system, patterns emerge. In this plot we'll use clustered heatmaps to graph the numbers from the Blood Bowl Fantasy football game, and see what patterns emerge!
...
Main source for this scheme is [@dode17], but the bash/dash/hybrid/stunty categorization is widespread, for example at [@amiral17], [@schlice18a], and [@breidr15].

It will be interesting to compare the patterns in the data with this categorization.
We'll return to this later, first we need to prep the data.
...
(I performed the analysis for the older divisions using the 2016 ruleset as well, the plots can be found [here](link_to_all_four_plots). )

The blog post from Schlice makes heavy use of functional programming using R's `purrr` package. This allows us to write a function and have this function work in parallel on a list of objects, and have it return the results also in list form. 
...
p1 + p2 + plot_layout(widths = c(1, 6)) + plot_annotation(title = 'BB2020 team typology')
```
...
The difference between Bash and Hybrid appears not so clear cut: Are Norse and Chaos Chosen bash or hybrid?
Agile and Stunty teams are very distinct. Underworld in the BB2020 rules cluster right in between Goblin and Skaven teams, I ended up classifying them as Agile.
