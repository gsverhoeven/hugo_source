...
The cool thing about DAGitty is that we can use a point-n-click interface to draw the diagram, and then export code that contains an exact description of the graph. You can also view the [graph for this blog post in DAGitty](http://dagitty.net/dags.html?id=whqGBx):
...
The graph comes with the following story:
...
In the experiments we analyze, two samples were taken, one for the PCR test and one for the antigen test.

Now that we now a bit about the measurement proces, lets look at how the accuracy of the test is quantified.


# Quantifying the accuracy of an COVID-19 test

The test is a binary test, either it detects the infection or it does not. For this type of outcome, two concepts are key: the **sensitivity** (does it detect COVID when you are infected) and **specificity** of the test (does it ONLY detect COVID, or also other flu types or even unrelated materials). 
...
There exists also a version of the Roche rapid antigen kit that comes with nasopharyngeal swabs.
...
They report that for the subset of samples with high viral load (ct_value < 30) **AND** who presented within 7 days of symptom onset, they found a sensitivity of 95.8% (CI95% 90.5-98.2). The percentage is based on N = 640 persons.
...
In the same subgroup of high viral load, sensitivity of the nasal swab test is 7% lower than the nasopharyngeal swab test. But how do we have to weigh this evidence? N = 92 is not so much data.
...
est_p <- rbinom(10000, sample_size, p=0.85)/sample_size

ggplot(data.frame(est_p), aes(x = est_p)) +
  geom_histogram()
```
We can see that the observed value in the Berlin dataset is consistent with the value we think is most plausible given our domain knowledge. 

So to conclude: a sizeable portion of the reduction in sensitivity is caused by the way the sample is taken, the swab effect, not by using a different measurement technique (PCR vs flow test)!
...
To do the PCR test you need a lab with skilled people, equipment such as PCR devices and pipets, and some time, as the process takes at least a few hours to complete. The advantage of an antigen test, such as the kit from Roche, is to have a low-tech, faster alternative that can be self-administered.
...
* [Seaman et al 2019](https://pubmed.ncbi.nlm.nih.gov/31400670/): Self-collected compared with professional-collected swabbing in the diagnosis of influenza in symptomatic individuals: A meta-analysis and assessment of validity
