...
g <- dagitty('dag {
bb="0,0,1,1"
Infected [exposure,pos="0.153,0.246"]
antigen_test [outcome,pos="0.428,0.530"]
close_contact_with_infected [pos="0.238,0.534"]
nose_swab [pos="0.335,0.428"]
pcr_test [outcome,pos="0.430,0.353"]
symptoms [pos="0.294,0.249"]
virus_buildup [pos="0.222,0.247"]
Infected -> virus_buildup
close_contact_with_infected -> nose_swab
...
These particles either do or do not cause symptoms. Whether there are symptoms will likely influence the decision to start a measurement (do a nose swab), but there will also be people without symptoms that will be measured (i.e. if a family member was tested positive).

Then given the sample, we can do either a PCR test or an antigen test.
...
How does this work? PCR can amplify, in so-called cycles, really low quantities of viral material in a biological sample. The amount of cycles of the PCR device needed to reach a threshold of signal is called the cycle threshold or **Ct value**. The less material we have in our sample, the more cycles we need to amplify the signal to reach a certain threshold. Because the amplification is an exponential process, we get a linear inverse (negative) relationship between **ct_value** and **viral_load** if we take the log of the number of virus particles. For example, $10^6$ particles is a viral load of 6 on the log10 scale.
...
From the data we also see that before the antigen test to work about half of the time, the **nasopharyngeal swab** needs to contain around $5 \cdot 10^5$ viral particles (log10 scale 5.7), and for it to work reliably, we need around $10^7$ particles ("high" viral load) in the **nasopharyngeal swab** (PM or combined NP/OP?). This last bit is important: the researchers did not measure the viral load in the nasal swabs used for the antigen test, these could well be lower. 
...
```{r}
df_pcr_pos %>% filter(viral_load >= 5.7 & days_of_symptoms) %>%
  summarize(sensitivity = mean(mm_value), 
            N = n())
```
...
These results are consistent with a recent systematic study by Lee et al, who find that
...
This is important to realize: a large portion of the reduction in sensitivity is caused by the way the sample is taken, not by using a different device!
...
We also found that even PCR is not able to detect all infected persons, it all depends on how much virus is present, and how the sample is obtained. From the analysis, it is clear that the rapid Antigen tests need more virus present to reliably detect infection. It is ALSO clear that the test is highly specific, with less than 1% false positives. Note that a false positive rate of 1% still means that in a healthy population of 1000, 10 are falsely detected as having COVID-19. 
Finally, we find that the nasal swabs are less sensitive than the nasopharyngeal ("through the nose") swabs, this reduces sensitivity by 5-10% (Igloi et al 2021, Lee et al 2021). 
...
Zsὁfia Iglὁi, Jans Velzing, Janko van Beek, David van de Vijver, Georgina Aron,  Roel Ensing, KimberleyBenschop,    Wanda   Han,    Timo   Boelsums,   Marion   Koopmans,    Corine   Geurtsvankessel,   Richard   Molenkamp

* Performance of Saliva, Oropharyngeal Swabs, and Nasal Swabs for SARS-CoV-2 Molecular Detection: a Systematic Review and Meta-analysis (https://journals.asm.org/doi/epub/10.1128/JCM.02881-20)
Rose A. Lee, Joshua C. Herigona, Andrea Benedetti, Nira R. Pollock, Claudia M. Denkinger
