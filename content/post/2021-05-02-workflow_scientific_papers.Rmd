...
header:
  image: headers/wilhelm-gunkel-di8ognBauG0-unsplash.jpg
  preview: no
---
...
(This blog post is mainly for my future self, and for people that ask me about my workflow for scientific papers.
Please contact me if you spot ways to improve this!)
...
Because author instructions are typically **REALLY** boring to read, a quick visual way to find out whats needed is to download a few **recent** (because change is the only constant), **open access** (no fuss with paywalls etc), and **highly cited** (this must mean they did something right with their paper, right?) example papers.
...
Another cool feature is that your references library is stored online, and your local Zotero install synchronizes with it, so you no longer need to move around library files.
Go to [the Zotero website](zotero.org) to download, install and configure Zotero.
...
(This snippet comes from [Dewey Dunnington's blog](https://fishandwhistle.net/post/2020/getting-started-zotero-better-bibtex-rmarkdown/))
...
For example, the journal "Health Services Research" want the references in the main text to be numbered, and the reference list sorted in the order of appearance, and formatted according the APA format (whatever that is).
...
This one is for our collaborators, who we feel sorry for, because the miss out on all the Rmarkdown fun, and need to work with software like Microsoft Word. And for some journals, that force us to submit our manuscript as a Word file.
...
The great thing, now I have this blog post, I can forget about all this stuff, and finally get to the scientific writing part! 
